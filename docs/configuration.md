...
Vite の変換結果では、巨大なグローバル辞書型を各 SFC に重複して展開しないように、`$locale.env` / `$l.env` は広い runtime 型になります。Vue Language Tools / Volar では詳細なグローバル辞書型を使うため、エディタ補完と `vue-tsc` の型検査は維持されます。
